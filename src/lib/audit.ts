...
export function createAuditLogger(config: OpenGateConfig, hooks?: AuditWriteHooks): AuditLogger | null {
...
export function createBufferedAuditLogger(
  ...
  async function flushQueue(retryOnFailure: boolean) {
    if (flushing) {
      await flushing;
      return;
    }
...
export function createAuditSink(auditConfig: AuditConfig): AuditSink {
...
export function createSqliteAuditSink(auditConfig: Pick<AuditConfig, "sqlitePath">): AuditSink {
  ...
  const transaction = db.transaction((events: AuditEvent[]) => {
...
export function createPostgresAuditSink(auditConfig: Pick<AuditConfig, "postgresUrl" | "postgresTable">): AuditSink {
