...
export async function authorizeRequest(config, policy, request) {
    ...
    if (jwtIdentity && apiKeyIdentity) {
        const matches = jwtIdentity.organizationId === apiKeyIdentity.organizationId &&
...
function extractJwtToken(config, request) {
...
function extractApiKey(config, request) {
...
async function verifyJwt(config, token) {
    ...
    for (const claim of issuerConfig.requiredClaims ?? ["iss", "aud", "exp", "sub", "unique_user_id"]) {
        if (claim === secondaryIdentifierClaim &&
    ...
    return {
        ...
        identity: {
            ...
            rateLimitSubject: `jwt:${(config.identityContext.globalUniqueness ?? "global") === "global"
...
function verifyApiKey(config, rawApiKey) {
...
function readStringClaim(payload, claimName) {
    const value = payload[claimName];
    return typeof value === "string" && value.length > 0 ? value : null;
}
function parseScopes(scopeClaim) {
    if (Array.isArray(scopeClaim)) {
        return scopeClaim.filter((item) => typeof item === "string" && item.length > 0);
    }
...
function createJwtSnapshot(config, claims) {
...
function enforceScopes(policy, identity) {
...
function deny(identity, statusCode, message, blockReason, jwtClaimSnapshot) {
    return {
        allowed: false,
        identity,
        statusCode,
...
function findUser(config, organizationId, userId) {
    return config.users?.find((user) => user.organizationId === organizationId && user.id === userId) ?? null;
}
export function createApiClientConfig(input) {
...
export function createApiKeyVersionConfig(input) {
    return {
        ...input,
        keyHash: hashApiKey(input.rawKey)
    };
}
function findApiKeyMatch(clients, keyHash) {
    const now = new Date();
...
function isKeyVersionActive(keyVersion, now) {
