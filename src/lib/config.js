...
const ALLOWED_JWKS_ALGORITHMS = ["RS256", "RS384", "RS512", "PS256", "PS384", "PS512", "ES256", "ES384", "ES512", "EdDSA"];
...
export function validateConfigDetailed(raw, baseDir = process.cwd()) {
...
export function migrateConfig(raw, baseDir = process.cwd()) {
    const report = validateConfigDetailed(raw, baseDir);
    if (!report.ok) {
        throw new ConfigValidationError(report.issues);
    }
...
function normalizeApiClient(client) {
...
function normalizeUser(user, organizations) {
...
function collectMigrationWarnings(raw) {
    const warnings = [];
    if (!raw || typeof raw !== "object") {
        return warnings;
    }
    const root = raw;
    const jwtSection = root.jwt && typeof root.jwt === "object" ? root.jwt : null;
    const apiKeysSection = root.apiKeys && typeof root.apiKeys === "object" ? root.apiKeys : null;
...
function validateAuditClaimAllowList(configuredClaims, jwtIssuers, identityClaim) {
    const supportedClaims = new Set(DEFAULT_AUDIT_CLAIMS);
...
function validateStorageBackends(config) {
