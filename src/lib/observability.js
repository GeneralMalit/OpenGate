...
export function createOpenGateTelemetry(config, logger) {
...
export function createConsoleLoggerAdapter() {
    return {
        emit(event) {
...
export function resolveOperationalPaths(config) {
    const observability = config.observability ?? {};
    return {
...
function getRouteMetric(routeMetrics, routePolicyId) {
...
async function emit(logger, event) {
    if (!logger) {
        return;
    }
