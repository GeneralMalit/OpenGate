...
export async function buildExampleApp(configPathOrConfig: string | OpenGateConfig = DEFAULT_CONFIG_PATH) {
  ...
  app.get("/", async (request, reply) => {
    ...
    reply.type("text/html").send(renderHomePage(hasCookie, cookieName, appVersion));
  });
  ...
  app.post("/login", async (request, reply) => {
    ...
    if (!user) {
      reply.code(401).type("text/html").send(renderHomePage(false, cookieName, appVersion, "Invalid demo credentials"));
      return;
    }
...
if (isMainModule) {
  const app = await buildExampleApp();
  const host = "127.0.0.1";
  const port = 3000;

  app.listen({ host, port })
    .then(() => {
      app.log.info(`OpenGate example listening on http://${host}:${port}`);
    })
    .catch((error) => {
      app.log.error(error);
      process.exit(1);
